...
export function sendData(data) {
	return  dispatch => {
		...
		post(obj.url.createPath({ server_name: 'exclusionCollab', action_name: 'SaveData' }), postData)
...
export function updateReason(reason){
	return {
		type: Constants.UPDATE_REASON_EXCL_COLLAB,
		reason
	};
}
...
export function getData(newPage, newSearch) {
	return dispatch => {
		...
		post(obj.url.createPath({ server_name: 'exclusionCollab', action_name: 'Collaborators' }), postData)
